...
public class WorldActivity extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        switch (id) {
            ...
            case (R.id.nav_open_nbt_by_name): {
                onOpenNBTByNameClicked(drawer);
                break;
            }
            case (R.id.nav_search_leveldb_key): {
                onSearchLevelDBKeys(drawer);
                break;
            }
            default:
                Log.d("UnknownNavigationID", "pressed unknown navigation-item in world-activity-drawer");
                break;
        }
    ...
    private void onOpenNBTByNameClicked(DrawerLayout drawer) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setView(keyEditText)
                .setPositiveButton(R.string.open, (dialog, id) -> changeContentFragment(() -> {
    ...
    private void onSearchLevelDBKeys(DrawerLayout drawer) {
        final EditText keyEditText = new EditText(this);
        keyEditText.setEms(16);
        keyEditText.setMaxEms(32);
        keyEditText.setHint(R.string.leveldb_key_here);

        final TextView resultText=new EditText(this);
        resultText.setTextIsSelectable(true);
        resultText.setEms(16);
        resultText.setMaxEms(32);
        resultText.setMaxLines(32);
        resultText.setHorizontalScrollBarEnabled(true);

        new AlertDialog.Builder(this)
                .setTitle(R.string.search_leveldb_key)
                .setView(keyEditText)
                .setView(resultText)
                .setNeutralButton("holder",null)
                .setNegativeButton(R.string.search, (dialog, id) -> changeContentFragment(() -> {
                    Editable keyNameEditable = keyEditText.getText();
                    String keyName = keyNameEditable == null
                            ? null : keyNameEditable.toString();
                    if (keyName == null || keyName.equals("")) {
                        ...
                        try {
                            ...
                            if (dbEntry == null) Snackbar.make(drawer,
                                ...
                                LinkedList<String> array=searchLevelDBKeys(keyName);
                                if(array==null){
                                    throw new Exception();
                                }
    ...
    public LinkedList<String> searchLevelDBKeys(final String keyName) throws IOException {
        ...
        if(keyName==null||keyName.isEmpty()){
            ...
            for(int i=0;i<keyName.length();i++){
                regex.append(toInsert);
                regex.append(keyName.charAt(i));
        ...
        try(Iterator iter=worldData.db.iterator()){
            String temp=null;
            Pattern pattern=null;
            Matcher matcher=null;
            for(iter.seekToFirst();iter.isValid();iter.next()){
                temp=iter.getKey().toString();
    ...
    public void openMultiplayerEditor() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setView(spinner)
                .setPositiveButton(R.string.open_nbt, (dialog, id) -> {
                    int spinnerIndex = spinner.getSelectedItemPosition();
                    String playerKey = players[spinnerIndex];

                    try {
                        final EditableNBT editableNBT = openEditableNbtDbEntry(playerKey);

                        changeContentFragment(() -> {
                            try {
                                openNBTEditor(editableNBT);
                            } catch (Exception e) {
                                alertErrorAndOpenWorldMap(e);
                            }
                        });

                    } catch (Exception e) {
                        e.printStackTrace();
                        var text = String.format(getString(R.string.failed_read_player_from_db_with_key_x), playerKey);
                        Snackbar.make(content, text, Snackbar.LENGTH_LONG)
    ...
    public void alertErrorAndOpenWorldMap(Exception e) {
        new AlertDialog.Builder(this)
                .setMessage(e.getMessage())
                .setCancelable(false)
                .setNeutralButton(android.R.string.ok,
                        (dialog, id) -> changeContentFragment(this::openWorldMap))
                .show();
    }
    ...
    public void onFatalDBError(WorldData.WorldDBException worldDBException) {
        ...
        new AlertDialog.Builder(this)
                .setMessage(R.string.error_cannot_open_world_close_and_try_again)
                .setCancelable(false)
    ...
    public String confirmContentClose;
    ...
    public void openNBTEditor(EditableNBT editableNBT) {
        ...
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.world_content, editorFragment)
                .addToBackStack(null)
                .commit();
    }
            ...
            final EditableNBT editableChunkData = new EditableNBT() {
                ...
                public String getRootTitle() {
                    ...
                    switch ((nbtChunkData).dataType) {
                        case ENTITY:
                            res = R.string.entity_chunk_data;
                            break;
                        case BLOCK_ENTITY:
                            res = R.string.tile_entity_chunk_data;
                            break;
                        default:
                            res = R.string.nbt_chunk_data;
                            break;
                    }
                    return String.format(Locale.getDefault(), format, getString(res), chunkX, chunkZ);
                }
