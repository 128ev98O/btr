...
public final class FlatLayers {
    ...
    private static final String KEY_WORLD_VERSION = "world_version";
    ...
    public static FlatLayers createNew(int biomeId, Layer[] layers) {
        ...
        ret.biomeId = biomeId;
        ret.encodingVersion = 6;
        ret.mLayers = layers;
        return ret;
    }

    @NonNull
    public static FlatLayers createNew(int biomeId, ArrayList<Layer> layers) {
        FlatLayers ret = new FlatLayers();
        ret.biomeId = biomeId;
        ret.encodingVersion = 6;
        ret.mLayers = new Layer[layers.size()];
        for (int i = 0; i < layers.size(); i++) {
            ret.mLayers[i] = layers.get(i);
        }
        return ret;
    }
    ...
    public String write() {
        try {
            ...
            root.put(KEY_WORLD_VERSION, "version.post_1_18");
