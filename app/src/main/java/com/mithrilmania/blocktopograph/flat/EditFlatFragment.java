...
public final class EditFlatFragment extends Fragment {
    ...
    public Map<Integer, Layer> getResultLayers() {
        var layers = new HashMap<Integer, Layer>();
        int count = -64;
        for (var item : mMeowAdapter.getItemList()) {
            layers.put(count, item);
            count += item.amount;
        }
        return layers;
    }
    ...
    private class MeowAdapter extends DragItemAdapter<Layer, MeowAdapter.MeowHolder> implements ListSwipeHelper.OnSwipeListener {
        ...
        private final ArrayList<Layer> blockList = new ArrayList<>(384);

        MeowAdapter(@NonNull AssetManager assets) {
            setItemList(blockList);
            this.assets = assets;
        }
        ...
        void loadDefault() {
            ...
            addItem(0, layer);
            layer = new Layer(BlockTemplates.getOfType("minecraft:dirt")[0], 2);
            addItem(0, layer);
        ...
        void insert(int index, Layer layer) {
            blockList.add(index, layer);
            notifyItemInserted(index);
        }

        void change(int index, Layer layer) {
            blockList.set(index, layer);
