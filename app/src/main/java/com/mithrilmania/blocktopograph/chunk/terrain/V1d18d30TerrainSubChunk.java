...
import com.mithrilmania.blocktopograph.utils.LittleEndianOutputStream;
...
public final class V1d18d30TerrainSubChunk extends TerrainSubChunk {
...
//            System.out.println("Count " + count + " ID " + id);
    ...
    public void setBlock(int x, int y, int z, int layer, @NonNull Block block) {
        ...
        if (mIsError || (layer > -4 && !mIsDualStorageSupported)) throw new RuntimeException();
    ...
    public void save(WorldData worldData, int chunkX, int chunkZ, Dimension dimension, int which) throws WorldData.WorldDBException, IOException {
        ...
        worldData.writeChunkData(chunkX, chunkZ, ChunkTag.TERRAIN, dimension, arr);
...
//        }
        ...
        public boolean setBlockIfSpace(int x, int y, int z, @NonNull Block block) {
            int code = -1;
        ...
        private static CompoundTag serializeBlock(@NonNull Block block) {
            ...
            if (blockType != null) {
                BlockProperty[] knownPropsKey = blockType.getKnownProperties();
                Object[] knownPropsValue = block.getKnownProperties();
                for (int i = 0; i < knownPropsKey.length; i++) {
                    BlockProperty prop = knownPropsKey[i];
                    switch (prop.getType()) {
                        case BYTE:
                            tags.add(new ByteTag(prop.getName(), (Byte) knownPropsValue[i]));
                            break;
                        case INT:
                            tags.add(new IntTag(prop.getName(), (Integer) knownPropsValue[i]));
                            break;
                        case STRING:
                            tags.add(new StringTag(prop.getName(), (String) knownPropsValue[i]));
                            break;
                    }
                }
            }

            var customPropeties = block.getCustomProperties().entrySet();
            for (var entry : customPropeties) {
                String name = entry.getKey();
                Object val = entry.getValue();
                if (val instanceof Byte) tags.add(new ByteTag(name, (Byte) val));
                else if (val instanceof Integer) tags.add(new IntTag(name, (Integer) val));
                else if (val instanceof String) tags.add(new StringTag(name, (String) val));
                else throw new RuntimeException("block state with unsupported type");
            }

            CompoundTag blockStates = new CompoundTag(PALETTE_KEY_STATES, tags);
            IntTag blockVersion = new IntTag(PALETTE_KEY_VERSION, 17879555);
            LinkedList<Tag> compoundTagValue = new LinkedList<>();
        ...
        private static Block deserializeBlock(@NonNull CompoundTag tag) {
            ...
            BlockType blockType = BlockType.get(name);
            Block.Builder builder = (blockType == null ? new Block.Builder(name) : new Block.Builder(blockType));
