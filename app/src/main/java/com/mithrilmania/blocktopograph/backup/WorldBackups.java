...
public class WorldBackups {
    ...
    private static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyyMMdd-HHmm-ss'-'");
    ...
    public void loadConfig() {
        ...
        try {
            JSONObject obj = new JSONObject(FileUtils.readFileToString(cfg, CHARSET));
    ...
    public void saveConfig() {
        ...
        try {
            ...
            FileUtils.writeStringToFile(getConfigFile(), obj.toString(4), CHARSET);
        } catch (Exception e) {
            Log.d(this, e);
        }
    }
    ...
    private void makeSureReadMeExists(String readMe) {
        ...
        try {
            FileUtils.writeStringToFile(file, readMe, CHARSET);
        } catch (IOException e) {
            Log.d(this, e);
        }
    }
    ...
    public boolean createNewBackup(String name, Date time) {
        ...
        File zfile = IoUtil.getFileWithFirstAvailableName(bakDir,
                DATE_FORMAT.format(time) + name, ".zip", "(", ")");
    ...
    public boolean restoreBackup(@NonNull Backup backup) {
        try (ZipFile zip = new ZipFile(backup.file)) {
            ...
            for (File file : files) if (!FileUtils.deleteQuietly(file)) return false;
